...
@socketio.on('connect')
def on_connect():
    user_id = request.args.get('token')  # 获取连接时携带的用户身份ID（token）
    if user_id:
        join_room(user_id)  # 将用户加入指定的房间（使用身份ID作为房间名）
        print(
            f'ZZXia Reminder: WebSocket Client {request.sid} connected with user ID: {user_id}')


@socketio.on('disconnect')
def on_disconnect():
    print('ZZXia Reminder: WebSocket Client disconnected')
...
def hook_hand():
    ...
    gan_cmd_full = gan_cmd_0 + ' ; ' + gan_cmd + ' tee -a ' + webhook_logfile + ' 2>&1'
    ...
    pipe = subprocess.Popen(gan_cmd_full, shell=True,
                            stdout=subprocess.PIPE, stderr=subprocess.PIPE, bufsize=1)
    for info in iter(pipe.stdout.readline, b''):
        info = info.decode("utf-8")
        socketio.emit('Message', info, room=hand_token)
        socketio.sleep(0.5)
